...
class GPIOHandler:
    ...
    def dataSimulation(self):
        ...
        # check if client server already available
        stat = True
        while stat:
        ...
        for i in range(self.DCount):
            ...
            # masuk
            GPIO.setup(self.loop1, GPIO.OUT)
            GPIO.output(self.loop1,GPIO.LOW)
            
            GPIO.setup(self.button, GPIO.OUT)
            GPIO.output(self.button,GPIO.LOW)
            sleep(1.5)

            GPIO.output(self.loop1,GPIO.HIGH)
            GPIO.output(self.button,GPIO.HIGH)
            
            GPIO.output(self.loop2,GPIO.LOW)
